...
#define KR_DEBUG_YELL(text) me->Yell(text, LANG_UNIVERSAL)
#else
#define KR_DEBUG_SAY(text)
#define KR_DEBUG_YELL(text)
#endif
...
public:
    ...
    struct boss_polydeucesAI : public ScriptedAI
    {
        boss_polydeucesAI(Creature* creature) : ScriptedAI(creature)
        {
            ...
            KR_DEBUG_SAY("CONSTRUCTOR: ### NEUE KR MODULE VERSION 2.0 - POLYDEUCES LOADED ###");
        ...
        void Initialize()
        {
            ...
            KR_DEBUG_YELL("### NEUE KR MODULE VERSION 2.0 GELADEN ###");
        ...
        void Reset() override
        {
            ...
            KR_DEBUG_SAY("ULTIMATE DEBUG: Polydeuces Reset() called!");
            events.Reset();
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            KR_DEBUG_SAY("DEBUG: POLYDEUCES SCRIPT IS RUNNING! Combat started!");
            
            if (_phase == PHASE_PREPARATION)
            {
                // Still in preparation - shouldn't happen but handle gracefully
                KR_DEBUG_SAY("DEBUG: Still in preparation phase!");
                return;
            }
        ...
        void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damagetype*/, SpellSchoolMask /*damageSchoolMask*/) override
        {
            ...
            KR_DEBUG_SAY("DEBUG: HP is " + std::to_string(me->GetHealthPct()) + "%");
            
            // Only trigger flight phases during ground combat (PHASE_ONE)
            if (_phase != PHASE_ONE || _isFlying)
            {
                KR_DEBUG_SAY("DEBUG: Wrong phase or already flying - phase=" + std::to_string(_phase) + " flying=" + std::to_string(_isFlying));
                return;
            }
            ...
            {
                KR_DEBUG_SAY("DEBUG: Triggering first flight phase at 66%!");
                StartFlightPhase();
            }
            else if (me->HealthBelowPctDamaged(33, damage) && _firstFlightDone && !_secondFlightDone)
            {
                KR_DEBUG_SAY("DEBUG: Triggering second and FINAL flight phase at 33%!");
                StartFlightPhase();
            }
            else if (me->HealthBelowPctDamaged(33, damage) && _secondFlightDone)
            {
                KR_DEBUG_SAY("DEBUG: No more flight phases - staying on ground!");
            }
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_LANDING_FALLBACK:
                        ...
                        KR_DEBUG_SAY("DEBUG: Landing fallback triggered!");
                        CompleteLanding();
                        break;
                }
            }
    ...
    private:
        ...
        void ExecuteTakeoffSequence()
        {
            ...
            KR_DEBUG_SAY("DEBUG: Starting air phase with Onyxia-style takeoff!");
            OnAirPhaseReached();
        }
        ...
        void OnAirPhaseReached()
        {
            ...
            KR_DEBUG_SAY("DEBUG: Air phase reached - starting events!");
        ...
        void StartFlightPhase()
        {
            ...
            KR_DEBUG_SAY("DEBUG: Flight phase starting!");
            me->AttackStop();
            me->SetReactState(REACT_PASSIVE);
        ...
        void OnWaypointReached(uint32 waypointId)
        {
            ...
            KR_DEBUG_SAY("DEBUG: Waypoint " + std::to_string(waypointId) + " reached! Starting bombardment!");
            ...
            if (_isFlying)
            {
                ...
                KR_DEBUG_SAY("DEBUG: Fireball bombardment started - " + std::to_string(_totalFireballs) + " fireballs!");
            }
        }
        ...
        void SpawnDione()
        {
            ...
            if (Creature* dione = me->SummonCreature(NPC_DIONE, spawnX, spawnY, spawnZ, spawnO, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 45000))
            {
                ...
                // Debug output
                KR_DEBUG_SAY("DEBUG: Dione spawned successfully!");
                ...
                if (Unit* target = dione->SelectNearestTarget(100.0f))
                {
                    ...
                    KR_DEBUG_SAY("DEBUG: Dione attacking target!");
                }
                else
                {
                    KR_DEBUG_SAY("DEBUG: Dione found no targets!");
                }
            ...
            else
            {
                ...
                // Debug output
                KR_DEBUG_SAY("DEBUG: Dione spawn failed! Retrying in 5 seconds...");
        ...
        void CheckDioneDeath()
        {
            ...
            KR_DEBUG_SAY("DEBUG: Stopped Dione death check");
                return;
            }
            ...
            if (Creature* dione = ObjectAccessor::GetCreature(*me, _currentDioneGUID))
            {
                ...
                else
                {
                    // Dione died - debug info
                        KR_DEBUG_SAY("DEBUG: Dione died! Initiating landing...");
                }
            }
            else
            {
                // Dione despawned - debug info  
                KR_DEBUG_SAY("DEBUG: Dione despawned! Initiating landing...");
            }
        ...
        void InitiateLanding()
        {
            ...
            KR_DEBUG_SAY("DEBUG: Initiating proper landing sequence!");
        ...
        void ExecuteFireballBombardment()
        {
            ...
            {
                CastScaledSpell(target, SPELL_FIREBALL);
                KR_DEBUG_SAY("DEBUG: Fireball " + std::to_string(_totalFireballs - _fireballsRemaining + 1) + "/" + std::to_string(_totalFireballs) + " fired!");
                _fireballsRemaining--;
                ...
                else
                {
                    ...
                    KR_DEBUG_SAY("DEBUG: Bombardment completed! Moving to next waypoint...");
        ...
        void CompleteLanding()
        {
            ...
            KR_DEBUG_SAY("DEBUG: Landing completed with proper MoveLand sequence!");
            ...
            {
                AttackStart(target);
                KR_DEBUG_SAY("DEBUG: Resuming combat with target!");
            }
            ...
            if (!_firstFlightDone)
            {
                ...
                KR_DEBUG_SAY("DEBUG: First flight phase completed! Next phase at 33% HP.");
            }
            else if (!_secondFlightDone)
            {
                _secondFlightDone = true;
                KR_DEBUG_SAY("DEBUG: Second and FINAL flight phase completed! No more flying!");
            }
            else
            {
                KR_DEBUG_SAY("DEBUG: ERROR - Unexpected flight phase completion!");
            }
